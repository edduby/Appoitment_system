...
{
   {
	...
	$connect = new mysqli($host,$db_user,$db_password,$db_name);
		...
		//is email i database?
		$result = $connect->query("SELECT time FROM appointment WHERE (doctorname='".$doctorname."' AND time='".$year."-".$mounth."-".$day." ".$hh.":".$mm.":00') OR (username='".$_SESSION['user']."' AND time='".$year."-".$mounth."-".$day." ".$hh.":".$mm.":00')");
		if(!$result) throw new Exception($connect->error);
		$how_many_mail = $result->num_rows;
			if($how_many_mail>0)
			{
			$all_ok=false;
			$_SESSION['e_login']="Term is unallowed in data base";
			}
				if($all_ok==true){
				//insert 
				echo "Good Register dupa"; 
				if($connect->query("INSERT INTO appointment VALUES ('','".$doctorname."','".$_SESSION['user']."','".$year."-".$mounth."-".$day." ".$hh.":".$mm.":00')"))
				{			
...
mysqli_report(MYSQLI_REPORT_STRICT);	
$connect = new mysqli($host, $db_user, $db_password, $db_name);
$result = $connect->query("SELECT doctorname FROM doctor WHERE branch='".$_SESSION['temp']."'");
echo "<select name='chosedoctor'>";
